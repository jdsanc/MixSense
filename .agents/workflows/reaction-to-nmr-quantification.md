...
- **Tool:** Call the `digitize_plot` MCP tool with `image_path` and `output_format="csv"`.
- **Action:** The MCP server handles VLM metadata extraction and CV pipeline automatically.
- **Output:** A numeric `.csv` or `.xy` spectrum file ready for downstream steps.
- **Decision:** If the digitized spectrum shows artifacts (axis labels captured as peaks, grid lines), re-run with an explicit `job_plan` overriding the problematic parameters.
...
- *Skill Reference:* `nmr-analysis` (`predict_products.py`)
...
- *Skill Reference:* `nmr-analysis` (`plot.py`)
...
- *Skill Reference:* `nmr-analysis` (`deconvolve.py`)
