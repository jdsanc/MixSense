...
_CSV_PATH = os.path.join(os.path.dirname(__file__), "..", "NMRBank", "NMRBank", "NMRBank_data_with_SMILES_156621_in_225809.csv")
...
def _parse_nmr_shifts(shift_text: str) -> List[float]:
    ...
    try:
        return [float(num) for num in numbers if float(num) >= -50 and float(num) <= 300]  # Reasonable NMR range
        ...
        for _, row in df.iterrows():
            ...
            if h1_shifts or c13_shifts:
                lut[key] = {
                    ...
                    "intensity": [1.0] * len(h1_shifts + c13_shifts)  # Default intensity
                }
