...
# ----------------------------
def run_llm_analysis(user_input: str, mixture_csv, model_name: str = "deepseek-ai/DeepSeek-V3:together") -> Tuple[str, str, pd.DataFrame, str]:
    """
    ...
    if not user_input.strip():
        return "Please provide a description of your chemistry analysis request.", "", pd.DataFrame(), "{}"
    ...
    if mixture_csv is not None:
        ...
        if not mixture_data["ppm"]:
            return "Invalid mixture CSV file. Please provide two columns: ppm, intensity.", "", pd.DataFrame(), "{}"
...
# ----------------------------
def run_llm_timeseries(user_input: str, files, model_name: str = "deepseek-ai/DeepSeek-V3:together") -> Tuple[str, str, pd.DataFrame, str]:
    ...
    if not user_input.strip():
        return "Please describe what species you want to analyze over time.", "", pd.DataFrame(), "{}"
