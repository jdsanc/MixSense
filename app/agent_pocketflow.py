...
def build_llm_graph(model_name="deepseek-ai/DeepSeek-V3:together", backend="auto"):
    ...
    def parse_natural_language(ctx):
        ...
        ctx["backend_choice"] = (
            task.backend_preference if task.backend_preference != "auto" else backend
        )
    ...
    def normalize_and_propose(ctx):
        ...
        if normalized_reactants:
            products = step_propose(
                normalized_reactants, ctx.get("reagents", ""), topk=5
            )
        ...
        ctx["steps"].append(
            {
                ...
                "status": "completed",
                "input": {
                    "raw_reactants": ctx.get("task").reactants,
                    "reagents": ctx.get("reagents", ""),
                },
                "output": {
                    "normalized_reactants": normalized_reactants,
                    "products": products,
                },
            }
        )
    ...
    def quantify_mixture(ctx):
        ...
        try:
            ...
            ctx["steps"].append(
                {"step": "quantify", "status": "failed", "error": str(e)}
            )
                ...
                if step["step"] == "normalize_propose":
                    ...
                    if refs_found:
                        summary_parts.append(
                            f"Found reference spectra for: {refs_found}"
                        )
...
"""
        ...
        try:
            ...
            ctx["narrative"] = (
                f"Analysis completed. Narrative generation failed: {str(e)}"
            )
