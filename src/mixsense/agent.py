...
from .tools_reactiont5 import propose_products, resolve_names_to_smiles
...
def normalize_smiles_list(text: str) -> List[str]:
    ...
    """
    # Prefer tools_reactiont5 resolver (which can use OPSIN/PubChem helpers)
    try:
        result = resolve_names_to_smiles(text)
        if result:
            print(f"Resolved via tools_reactiont5: {text} -> {result}")
            return result
    except Exception:
        print("tools_reactiont5 name resolution failed; using fallback map")
    ...
    name_to_smiles = {
        ...
        # Common mixture demo components
        "pinene": "CC1=CCC2CC1C2(C)C",           # alpha-pinene
